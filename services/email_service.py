...
def send_demo_request_email(
    ...
    html_content = Template(html_template).substitute(
        ...
        date=date,
        time=time,
        comments=comments if comments else "Info not given"
    )
...
Additional Notes: {comments if comments else "Info not given"}
...
async def send_interview_confirmation_emails(
    ...
    previous_rounds_section: str = None,
    is_first_round: bool = True,
    transcript_url: str = None,
    transcript_attachment_path: str = None,
) -> bool:
    """
    Send interview confirmation emails to applicant and interviewers.
    If previous_rounds_section is provided, it is shown to interviewers; otherwise
    is_first_round controls showing "This is the first round."
    AI transcript: use transcript_url (S3 link in body) or transcript_attachment_path (local file attach).
    """
    try:
        ...
        # Previous rounds / first round section for interviewer (always show)
        section_content = (previous_rounds_section or "").strip()
        if not section_content or is_first_round:
            section_content = "<p style=\"margin: 0;\"><strong>Previous rounds:</strong> This is the first round.</p>"
        transcript_note = ""
        if transcript_url:
            transcript_note = f'<p style="margin: 12px 0 0 0; font-size: 14px; color: #166534;"><strong>An AI interview was conducted.</strong> <a href="{transcript_url}">View transcript</a>.</p>'
        elif transcript_attachment_path and os.path.exists(transcript_attachment_path):
            transcript_note = '<p style="margin: 12px 0 0 0; font-size: 14px; color: #166534;"><strong>An AI interview was conducted.</strong> The transcript is attached to this email.</p>'
        previous_rounds_block = f"""
                    <div class="details-box" style="margin-top: 16px;">
                        <h3 style="margin: 0 0 12px 0; font-size: 16px; color: #1e40af;">📋 Previous Rounds</h3>
                        {section_content}
                        {transcript_note}
                    </div>
            """
        ...
        if not transcript_url and transcript_attachment_path and os.path.exists(transcript_attachment_path):
            attachments.append((transcript_attachment_path, "AI_Interview_Transcript.txt"))
